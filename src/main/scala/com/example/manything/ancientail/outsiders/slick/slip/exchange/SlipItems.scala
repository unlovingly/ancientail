...
class SlipItems(tag: Tag) extends Table[PolishedExchangeSlipItem](tag, "exchange_slip_items") {
  ...
  def * =
    (identity.?, productId, amount, price, slipId) <> ((PolishedExchangeSlipItem.apply _).tupled, PolishedExchangeSlipItem.unapply)
}
